...
with exec_summary_tab3:
    ...
    # === ENHANCED SUCCESS METRICS WITH VISUAL TRACKING ===
    st.markdown("### 🎯 KPI Performance Dashboard")

    with st.container(border=True):
        # Create progress visualization for key metrics
        col_kpi1, col_kpi2, col_kpi3, col_kpi4 = st.columns(4)

        with col_kpi1:
            current_mrr = 8.2
            target_mrr = 12.1
            mrr_progress = (current_mrr / target_mrr) * 100
            st.metric("MRR Progress", f"{mrr_progress:.1f}%", f"${current_mrr}M / ${target_mrr}M")
            st.progress(mrr_progress / 100)

        with col_kpi2:
            current_churn = 3.2
            target_churn = 2.0
            churn_improvement = ((current_churn - target_churn) / current_churn) * 100
            st.metric("Churn Reduction", f"{churn_improvement:.1f}%", f"{current_churn}% → {target_churn}%")
            st.progress((current_churn - target_churn) / current_churn)

        with col_kpi3:
            current_conversion = 22
            target_conversion = 28
            conversion_progress = (current_conversion / target_conversion) * 100
            st.metric("Conversion Rate", f"{conversion_progress:.1f}%", f"{current_conversion}% → {target_conversion}%")
            st.progress(conversion_progress / 100)

        with col_kpi4:
            current_expansion = 2.1
            target_expansion = 5.5
            expansion_progress = (current_expansion / target_expansion) * 100
            st.metric("Market Expansion", f"{expansion_progress:.1f}%", f"${current_expansion}M → ${target_expansion}M")
            st.progress(expansion_progress / 100)
    ...
    with st.container(border=True):
        performance_summary = {
            "🎯 Strategic Priorities": [
                "Revenue optimization initiatives showing $5.16M annual opportunity",
                "Customer intelligence reveals 347 high-risk accounts needing intervention",
                "Market expansion strategy identifies $15.1M TAM across 7 markets",
                "Data quality improvements prevent $500K in poor decision-making"
            ],
            "⚡ Quick Wins (30 Days)": [
                "Launch customer success intervention program",
                "A/B test Pro plan pricing increase (8% target lift)",
                "Deploy Basic tier upselling campaign for power users",
                "Implement trial-to-paid conversion optimization"
            ],
            "🚀 Growth Drivers (90 Days)": [
                "Australia market entry execution ($900K revenue target)",
                "Annual billing conversion campaign launch",
                "Enterprise trial extension program with dedicated CSM",
                "Data quality monitoring framework deployment"
            ],
            "🎯 Success Criteria": [
                "Achieve 68% retention rate for at-risk customers",
                "Maintain <5% churn increase during pricing changes",
                "Reach 25% trial-to-paid conversion rate",
                "Generate $900K ARR from Australia market within 12 months"
            ]
        }
    ...
    st.markdown("### 🎖️ Key Performance Indicators - Annual Targets")

    # Create a final summary chart of annual targets
    ...
    kpi_chart_data["Improvement Required"] = (
        (kpi_chart_data["Annual Target"] - kpi_chart_data["Current"]) /
        kpi_chart_data["Current"] * 100
    ).round(1)

    # Display the final KPI summary
    st.dataframe(
        kpi_chart_data[["Metric", "Current", "Annual Target", "Improvement Required"]].style.format({
            "Improvement Required": "{:.1f}%"
        }),
        use_container_width=True
    )

    st.markdown("---")
    st.markdown("*This comprehensive dashboard provides executives with actionable insights to drive $5.16M in annual revenue optimization through data-driven decision making.*")
